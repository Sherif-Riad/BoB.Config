...
function buildpage() {//you must call parsedata before buildpage, otherwise it will build with default Data
  ...
  if (bUseHardGroupLimits) {
    document.getElementById("dinoh").textContent = "Group Hard-Limit";
  } else {
    document.getElementById("dinoh").textContent = "Group Soft-Limit";
  }
  var EventTable = document.getElementById('WorldEventBuffsToNotUse');
  var eventdat = EventTable.rows[0].cells[0].childNodes;
  if (eventdat.length > 1) {
    for (var ele = eventdat.length - 1; ele > 0; ele--) {
      if (eventdat[ele].nodeType === 1 && eventdat[ele].tagName.toLowerCase() === 'input') {
        eventdat[ele].remove();
      }
    }
  }
  ...
  //var AIdat = AITable.rows[0].cells[0].childNodes;
  //if (AIdat.length != 1) {
  ...
  //    AITable.rows[0].cells[0].appendChild(btn);
  //  }
  //}
  ...
  // Clean up the existing input buttons if there are any
  if (AIdat.length != 1) {
    for (var ele = AIdat.length; ele-- > 1;) {
      // Check if the node is an element node (nodeType 1 is for element nodes)
      if (AIdat[ele].nodeType === 1 && AIdat[ele].tagName.toLowerCase() == 'input') {
        AIdat[ele].remove();
      }
    }
  }

  // Assuming DisabledAITypes.count != 0
  if (DisabledAITypes.count != 0) {
    for (var i in DisabledAITypes) {
      if (i == "length" || i === 'item') { break; }

      // Create a new button for each disabled AI type
      var btn = document.createElement('input');
      btn.type = 'button';
      btn.setAttribute('onclick', 'this.remove()');
      btn.value = DisabledAITypes[i];

      // Append the button to the first cell of the first row
      AITable.rows[0].cells[0].appendChild(btn);
    }
  }
  ...
  // Empty first master row "ALL" to avoid any conflict
  // Iterate through all rows starting from the second row (skip the header row)
  for (let i = 1; i < dinotable.rows.length; i++) {
    const row = dinotable.rows[i];

    // If the first cell contains "ALL", empty the subsequent input fields
    if (row.cells[0].textContent.trim() === "ALL") {
  ...
  for (var i in dinotable.rows) {
    ...
    if (i !== 0) {
      ...
      // Enhanced version by Anexmos
      if (dname == 'Creature' || dname == 'ALL') { continue; }//so we dont attempt to process the first row
      for (crname in CreatureLimits) {
        if (CreatureLimits[crname][0] === dname) {
          valid = true;
          dinotable.rows[i].cells[1].firstChild.value = CreatureLimits[crname][2]; // Update cell 1
          dinotable.rows[i].cells[2].firstChild.value = CreatureLimits[crname][1]; // Update cell 2
          dinotable.rows[i].cells[4].firstChild.value = CreatureLimits[crname][3]; // Update cell 4
          dinotable.rows[i].cells[5].firstChild.value = CreatureLimits[crname][4]; // Update cell 5
          break; // Exit loop once the matching condition is found
        }
      }
      if (!valid) {//somehow we dont have a creature set in the config so build a default
        dinotable.rows[i].cells[1].firstChild.value = "20";
        dinotable.rows[i].cells[2].firstChild.value = "100.0";
      ...
      for (var grname in GrowthLimits) {
        if (GrowthLimits[grname][0] == dname) {
          valid = true;
          if (GrowthLimits[grname][1] <= 50) { dinotable.rows[i].cells[3].firstChild.value = GrowthLimits[grname][1]; } else {
            dinotable.rows[i].cells[3].firstChild.value = 50.0;
            InternalDebug("WARN: Creature " + dname + " growth exceeded game max, setting to game max");
          }
          break;
        }
      }
      if (!valid && !GrowthLimit == 0 && GrowthLimit <= 50) {
  ...
  // Updated version by Anexmos
  var commands = document.getElementById("commands");
  for (i in commands.rows) {
    var valid = false;
    if (i === "length" || i === 'item') { break; }
    if (i != 0) {
      // Directly access the 'div.name' element in the first column
      var cname = commands.rows[i].cells[0].querySelector('.name').innerHTML;

      var crank = commands.rows[i].cells[1].getElementsByTagName('select')[0]; // Access the select element
      crank.innerHTML = ""; // Clear existing options
      crank.appendChild(rankfragment.cloneNode(true)); // Append new options

      for (var perms in AdminCommandRules) {
        if (AdminCommandRules[perms][0] === cname) {
          valid = true;
          crank.value = AdminCommandRules[perms][1]; // Set the value based on the permissions
          break;
        }
      }
      if (!valid) {
        InternalDebug('WARN: ' + cname + " was missing from the config. setting to blank");
        crank.value = ' '; // Set to blank if no valid configuration is found
      }
    }
  }
  ...
  // Enhanced version for bulk api request by Anexmos
  // Step 1: Collect all the Steam IDs
  let steamIds = [];
  for (let newstaff in playersettings) {
    steamIds.push(playersettings[newstaff][0]); // Assuming Steam ID is in the first column of playersettings
  }

  // Create a map to store the index of each Steam ID
  const playerIndexMap = {};
  playersettings.forEach((playerSetting, index) => {
    playerIndexMap[playerSetting[0]] = index; // Map Steam ID to its row index
  });

  // Step 2: Perform batched API requests (10 IDs per batch)
  async function fetchSteamProfiles(steamIds) {
    ...
    const batchSize = 10; // Number of Steam IDs per batch
    for (let i = 0; i < steamIds.length; i += batchSize) {
      const idsBatch = steamIds.slice(i, i + batchSize); // Create a batch of IDs
      await fetchBatch(idsBatch); // Fetch data for this batch
    }

    // Step 5: Sort the table by original Steam ID order
    sortTableByOriginalOrder(steamIds);
  }

  // Function to fetch a single batch of Steam profiles
  async function fetchBatch(idsBatch) {
    const idString = idsBatch.join(','); // Join IDs into a comma-separated string
    ...
    try {
      ...
      if (data.response && data.response.players) {
        // Process each player returned in the API response
        data.response.players.forEach((player) => {
          const steamId = player.steamid; // Get the Steam ID from the API response
          const rowIndex = playerIndexMap[steamId]; // Get the correct index using the Steam ID

          if (rowIndex !== undefined) {
            const playerSetting = playersettings[rowIndex]; // Get the player setting for this index

            // Step 3: Populate the table with Steam profile data
            const row = stafftab.insertRow(-1);
            const div = document.createElement('div'); // Add rank options
            div.appendChild(rankfragment.cloneNode(true));
            ...
            // Cell for Steam ID input
            cell0.innerHTML = `<input type="number" value="${playerSetting[0]}" onchange="prepareBulkIds()" onwheel="this.blur()">`;
            cell7.innerHTML = '<a href="#" target="_blank">Retrieving Steam Name...</a>';

            // Other cells
            cell1.innerHTML = `<select name="Ranks">${div.innerHTML}</select>`;
            cell1.firstChild.value = playerSetting[1];
            cell2.innerHTML = `<input type="text" value="${playerSetting[2]}">`;
            cell3.innerHTML = `<input type="color" value="${playerSetting[3]}" onchange="ValidateColor(this)">`;
            cell4.innerHTML = `<input type="text" pattern="^#+([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$" onchange="ValidateColor(this)" size="7" maxlength="7" value="${playerSetting[3]}" style="vertical-align: middle">`;
            cell5.innerHTML = '<input type="button" value="X" onclick="StaffRemoveRow(this)">';
            cell6.innerHTML = '<input type="button" value="+" onclick="StaffAddRow(this)">';

            // Update the Steam profile data
            if (player && player.profileurl && player.personaname) {
              cell7.firstChild.href = player.profileurl; // Set the profile URL
              cell7.firstChild.innerHTML = player.personaname; // Set the Steam Name
            } else {
              cell7.firstChild.innerHTML = 'Profile not found'; // Fallback text if profile is missing
            }
          } else {
            console.warn(`No matching index found for Steam ID ${steamId}`);
          }
        });
      } else {
  ...
  // Step 5: Sort the table by the original order of Steam IDs
  function sortTableByOriginalOrder(steamIds) {
    const rows = Array.from(stafftab.rows); // Convert HTMLCollection to an array
    const header = rows.shift(); // Remove the header row if present

    // Create a map of Steam ID to its index in the original steamIds array
    const idIndexMap = steamIds.reduce((map, id, index) => {
      map[id] = index;
      return map;
    }, {});

    // Sort rows based on their Steam ID order in the original steamIds list
    rows.sort((rowA, rowB) => {
      const steamIdA = rowA.cells[0].firstChild.value; // Get Steam ID from the first cell
      const steamIdB = rowB.cells[0].firstChild.value; // Get Steam ID from the first cell

      // Use the mapped indices for comparison
      const indexA = idIndexMap[steamIdA] ?? Infinity; // Default to Infinity if not found
      const indexB = idIndexMap[steamIdB] ?? Infinity; // Default to Infinity if not found

      // Stable sort: if indices are the same, maintain original order
      return indexA - indexB;
    });

    // Re-add the header row (if any)
    if (header) stafftab.appendChild(header);

    // Re-add rows to the table in sorted order
    rows.forEach((row) => stafftab.appendChild(row));
  }

  // Step 4: Call fetchSteamProfiles with the collected IDs
